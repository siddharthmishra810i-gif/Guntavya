...
function getFallbackItinerary(destination: string, daysLength: number = 3, personality: string = "Adventure Seeker", mood: string = "Lively") {
...
app.post("/api/plan", async (req, res) => {
  const { destination, days = 3, personality = "Standard", mood = "", exploreMode = "standard" } = req.body;
  try {
...
9. situational local translations (emergency, medical, list allergy, local slang)`,
    ...
    const fallback = getFallbackItinerary(destination, Number(days) || 3, personality, mood);
    res.json({
      ...fallback,
      isQuotaFallback: true,
